...
class InvoicesController extends BaseGuestController
{
    ...
    public function view($invoice_id): void
    {
        ...
        $this->layout->set(['invoice_id' => $invoice_id, 'invoice' => $invoice, 'items' => Item::query()->where('invoice_id', $invoice_id)->get(), 'invoice_tax_rates' => InvoiceTaxRate::query()->where('invoice_id', $invoice_id)->get(), 'enable_online_payments' => get_setting('enable_online_payments'), 'legacy_calculation' => config_item('legacy_calculation')]);
