@php namespace Modules\Settings\Views; @endphp
<script>
    $().ready(function () {
        $('#btn-submit').click(function () {
            $('#form-settings').submit();
        });
        $('[name="settings[default_country]"]').select2({
            placeholder: '@@lang('country')',
            allowClear: true
        });
        if(window.ls) {
            // Memorise active tab
            $('a[data-toggle="tab"]').on('show.bs.tab', function(e) {
                localStorage.setItem(window.ls, $(e.target).attr('href'));
            });
            var activeTab = localStorage.getItem(window.ls);
            if(activeTab) {
                $('#settings-tabs a[href="' + activeTab + '"]').tab('show');
            }
        }
    });

    window.ls = typeof(localStorage) != 'undefined' ? 'activeTab-settings' : '';
    if(window.ls) {
        const lsother = window.ls + '-other';
        // Become from other page, Return to general tab (Clear memory)
        if(document.referrer != '{{ url('settings') }}') {
            // Note: when become from other page & refresh it, the originaly referrer is returned but show last choosen tab
            localStorage.setItem(lsother, (localStorage.getItem(lsother) ? parseInt(localStorage.getItem(lsother)) : 0) + 1);
            if(localStorage.getItem(lsother) == 1 && localStorage.getItem(window.ls)) {
                localStorage.removeItem(window.ls); // Clear tab memory
            }
        } else {
            $(window).on('unload', function() {
                localStorage.removeItem(lsother); // Clear memory
            });
        }
    }
</script>

<div id="headerbar">
    <h1 class="headerbar-title">@@lang('settings')</h1>
    @php $this->layout->loadView('layout/header_buttons', ['hide_cancel_button' => true]); @endphp
</div>

<ul id="settings-tabs" class="nav nav-tabs nav-tabs-noborder">
    <li class="active">
        <a data-toggle="tab" href="#settings-general">@@lang('general')</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-invoices">@@lang('invoices')</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-quotes">@@lang('quotes')</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-taxes">@@lang('taxes')</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-email">@@lang('email')</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-online-payment">{{ @@lang('online_payment') }}</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-projects-tasks">@@lang('projects')</a>
    </li>
    <li>
        <a data-toggle="tab" href="#settings-updates">@@lang('updates')</a>
    </li>
</ul>

<form method="post" id="form-settings" enctype="multipart/form-data">

    @php _csrf_field(); @endphp

    <div class="tabbable tabs-below">

        <div class="tab-content">

            <div class="col-xs-12 col-md-8 col-md-offset-2">
                @php $this->layout->loadView('layout/alerts'); @endphp
            </div>

            <div id="settings-general" class="tab-pane active">
                @php $this->layout->loadView('settings/partial_settings_general'); @endphp
            </div>

            <div id="settings-invoices" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_invoices'); @endphp
            </div>

            <div id="settings-quotes" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_quotes'); @endphp
            </div>

            <div id="settings-taxes" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_taxes'); @endphp
            </div>

            <div id="settings-email" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_email'); @endphp
            </div>

            <div id="settings-online-payment" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_online_payment'); @endphp
            </div>

            <div id="settings-projects-tasks" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_projects_tasks'); @endphp
            </div>

            <div id="settings-updates" class="tab-pane">
                @php $this->layout->loadView('settings/partial_settings_updates'); @endphp
            </div>

        </div>

    </div>

</form>
