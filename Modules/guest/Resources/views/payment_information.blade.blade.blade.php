                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-md-7">
                            <h4>
                                @php _htmlsc(format_client($invoice)); @endphp
                            </h4>
...
@php if ($payment_method) {
    ...
    _htmlsc($payment_method->payment_method_name);
    @endphp</td>
                                        </tr>
@php
} @endphp
...
@php if (!empty($invoice->invoice_terms)) {
    @endphp
                        <div class="col-xs-12 text-muted">
                            ...
                            <h4>@@lang('terms')</h4>
                            <div>@php
    _htmlsc(nl2br($invoice->invoice_terms));
    @endphp</div>
                        </div>
@php
} @endphp
...
@php if ($payment_provider == null && !$disable_form) {
    @endphp
                <div>
...
@php
    foreach ($gateways as $gateway) {
        @endphp
                    <a class="list-group-item list-group-item-action" href="{{ url('guest/payment_information/form/' . $invoice->invoice_url_key . '/' . $gateway) }}">{{ ucwords(str_replace('_', ' ', $gateway)) }}</a>
@php
    }
    @endphp
                </ul>
