...
class StripeController extends BaseController
{
    ...
    {
        try {
            // Retrieve the Checkout Session from StripeController
            ...
            // +2 status: *_NO_PAYMENT_REQUIRED *_UNPAID
            // Is paid? (intent flow 'succeeded')
            if ($paid) {
                // Save the payment (visible in guest user)
                $this->load->model('payments/mdl_payments');
                $this->mdl_payments->save(null, ['invoice_id' => $invoice->invoice_id, 'payment_date' => date('Y-m-d'), 'payment_amount' => $session->amount_total / 100, 'payment_method_id' => get_setting('gateway_stripe_payment_method'), 'payment_note' => trans('online_payment_intent_id') . ': ' . $session->payment_intent]);
            }
            // paid / cancel (+other flow)
            // Admin (& error log) message
            $response = $paid ? '. livemode: ' . trans($session->livemode ? 'yes' : 'no') . ', currency: ' . $session->currency . ', amount: ' . $session->amount_received / 100 . ', fee: ' . $session->application_fee_amount / 100 . ', session ID: ' . $session->id : ($session->cancel ? $session->cancellation_reason : $session->last_payment_error);
            // Cancelled
            // User (& error) message
            ...
            log_message('error', strtr($response . ' user_msg: ' . $user_msg, ['<br>' => ' ']));
            // No br's
        } finally {
            $paid = is_bool($paid) ? $paid ? 'success' : 'info' : $paid;
            // Tweak to reuse (flashdata alert_*)
