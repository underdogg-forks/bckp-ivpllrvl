...
class ImportService extends BaseService
{
        ...
        while (($data = fgetcsv($handle, 1000, ',')) != false) {
            ...
            if ($row > 1) {
                ...
                foreach ($headers as $key => $header) {
                    ...
                    $db_array['invoice_url_key'] = $this->mdl_invoices->getUrlKey();
        ...
        while (($data = fgetcsv($handle, 1000, ',')) != false) {
            ...
            if ($row > 1) {
                ...
                if ( ! $record_error) {
                    ...
                    $global_discount = [
                        'amount' => 0.0,
                        // Only to prevent bug because mdl_items->save() call Mdl_Item_Amounts->calculate()
                        'percent' => 0.0,
                        // But no Global discount applied because all amount = 0
                        'item' => 0.0,
                        // This is Global invoice discount
                        'items_subtotal' => 0.0,
                    ];
     ...
     * @originalFile Import.php
     */
    public function recordImportDetails($import_id, $table_name, $import_lang_key, $ids)
    {
        foreach ($ids as $id) {
