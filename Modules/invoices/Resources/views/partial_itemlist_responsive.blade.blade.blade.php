        ...
        <div class="form-group details-box item">
            <div class="row">
                <div class="col-xs-12 col-sm-7 col-md-6 col-lg-5">
                    <div class="row">
                        <div class="col-xs-12 col-sm-1">
                            <button type="button" class="btn btn-link up" title="@@lang('move_up')"{{ $invoice_disabled }}>
                                <i class="fa fa-chevron-up"></i>
                            </button>
                            <button type="button" class="btn btn-link down" title="@@lang('move_down')"{{ $invoice_disabled }}>
                                <i class="fa fa-chevron-down"></i>
                            </button>
@php
...
@php
    ...
    @endphp
                        ...
                        <div class="col-xs-12 col-sm-11">
                            ...
                            <input type="hidden" name="item_task_id" class="item-task-id" value="@php
    ...
    @endphp">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="item_name" id="item_name_{{ $item->item_id }}" class="form-control" value="{{ _htmlsc($item->item_name) }}"{{ $invoice_disabled }}>
                            </div>
...
@php
    if ($invoice->sumex_id == '') {
        @endphp
                                ...
                                <textarea name="item_description" id="item_description_{{ $item->item_id }}" class="form-control"{{ $invoice_disabled }}>@php
        _htmlsc($item->item_description);
        @endphp</textarea>
@php
    } else {
        @endphp
                                <label for="item_date_{{ $item->item_id }}" class="input-group-addon ig-addon-aligned">@@lang('date')</label>
                                <input type="text" name="item_date" id="item_date_{{ $item->item_id }}" class="form-control datepicker" value="{{ format_date($item->item_date) }}"{{ $invoice_disabled }}>
@php
    }
    @endphp
                            </div>
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-lg-6">
                            <div class="input-group">
                                ...
                                <input type="text" name="item_quantity" id="item_quantity_{{ $item->item_id }}" class="form-control" value="{{ format_quantity($item->item_quantity) }}"{{ $invoice_disabled }}>
                            </div>
                            <div class="input-group">
                                <label for="item_product_unit_id_{{ $item->item_id }}" class="input-group-addon ig-addon-aligned">@@lang('product_unit')</label>
                                <select name="item_product_unit_id" id="item_product_unit_id_{{ $item->item_id }}" class="form-control"{{ $invoice_disabled }}>
                                    <option value="0">@@lang('none')</option>
@php
...
@php
    ...
    @endphp
                            <div class="input-group">
                                ...
                                <select name="item_tax_rate_id" id="item_tax_rate_id_{{ $item->item_id }}" class="form-control"{{ $invoice_disabled }}>
                                    <option value="0">@@lang('none')</option>
@php
