...
class CronController extends BaseController
{
    ...
    public function recur($cron_key = null)
    {
        ...
        if ($cron_key != get_setting('cron_key')) {
            log_message('error', '[CronController RecurringController InvoicesController] Wrong cron key provided! ' . $cron_key);
        ...
        foreach ($invoices_recurring as $invoice_recurring) {
            ...
            $invoice = $this->mdl_invoices->getById($source_id);
            ...
            $this->mdl_invoices->copyInvoice($source_id, $target_id, false);
            if (IP_DEBUG) {
            ...
            $this->mdl_invoices_recurring->setNextRecurDate($invoice_recurring->invoice_recurring_id);
            if (IP_DEBUG) {
            ...
            if (get_setting('automatic_email_on_recur') && mailer_configured()) {
                $new_invoice = $this->mdl_invoices->getById($target_id);
                ...
                if ( ! $email_template_id) {
                    log_message('error', '[CronController RecurringController InvoicesController] No email template set in the system settings!');
                    continue;
                }
                ...
                $attachment_files = $this->mdl_uploads->getInvoiceUploads($target_id);
                // Prepare the body
                ...
                if ($email_invoice) {
                    $this->mdl_invoices->markSent($target_id);
                } else {
                    log_message('error', '[CronController RecurringController InvoicesController] Invoice ' . $target_id . 'could not be sent. Please review your Email settings.');
                }
            } else {
                log_message('error', '[CronController RecurringController InvoicesController] Automatic_email_on_recur was not set or mailer was not configured');
            }
        }
        if (IP_DEBUG) {
