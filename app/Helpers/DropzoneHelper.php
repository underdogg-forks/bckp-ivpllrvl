...
{
    /**
     * @originalName _dropzone_html
     *
     * @originalFile dropzone_helper.php
     */
    public static function DropzoneHtml($read_only = true): void
    {
        ?>
<div class="panel panel-default no-margin">
    ...
    <div class="panel-body clearfix">
        ...
        <button type="button" class="btn btn-sm btn-default fileinput-button<?php
        echo $read_only ? ' hide" readonly="readonly' : '';
        ?>">
...
<?php
    ...
    /**
     * @originalName _dropzone_script
     *
     * @originalFile dropzone_helper.php
     */
    public static function DropzoneScript($url_key = null, $client_id = 1, $site_url = '', $acceptedExts = null): void
    {
...
acceptedExts    = '.<?php
        echo implode(',.', $content_types);
        ?>'; // allowed .ext1,.ext2,.ext3, ...
    ...
    function getIcon(fullname) {
        ...
        return '<?php
        echo base_url('assets/core/img/file-icons/');
        ?>' + fileIcon + '.svg';
    }
    ...
    var myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
        ...
        dictFileTooBig: `<?php
        _trans('upload_dz_invalid_file_size');
        ?>`,
        dictFileSizeUnits: {<?php
        _trans('upload_dz_size_units');
        ?>},
        dictRemoveFileConfirmation: `<?php
        _trans('delete_attachment_warning');
        ?>`,
        dictInvalidFileType: `<?php
        _trans('upload_dz_invalid_file_type');
        ?>`,
    ...
    myDropzone.on('complete', function (file) {
        ...
        if(file.xhr && file.xhr.responseURL.match(/sessions\/login/) !== null) {
            this.emit('error', file, `<?php
        _trans('upload_dz_disconnected');
        ?>`);
        }
    });
    ...
    myDropzone.on('error', function (file, message) {
        <?php
        echo (IP_DEBUG ? 'console.log("dropzone error", file, message, this);' : '') . PHP_EOL;
        ?>
...
<?php
    ...
    myDropzone.on('sending', function (file, xhr, formData) {
        ...
        formData.append('<?php
            ...
            ?>', Cookies.get('<?php
            echo config_item('csrf_cookie_name');
            ?>'));
    ...
    removeAllFilesButton.addEventListener('click', function (e) {
        ...
        e.stopPropagation();
        confirm(`<?php
            _trans('delete_attachments_warning');
