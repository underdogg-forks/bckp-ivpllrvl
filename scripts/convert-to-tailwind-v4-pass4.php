...
function convertFilePass4($filePath, $dryRun = false) {
    ...
    $content = preg_replace_callback($pattern, function($matches) use (&$changes) {
        ...
        $primaryPrefix = trim($matches[3]);
        $defaultPrefix = trim($matches[5]);
        ...
        // Combine
        $primaryFull = trim($primaryPrefix . ' ' . $primaryClasses);
        $defaultFull = trim($defaultPrefix . ' ' . $defaultClasses);
    ...
    $content = preg_replace_callback('/class="([^"]+)"/', function($matches) {
        $classes = preg_replace('/\s+/', ' ', trim($matches[1]));
