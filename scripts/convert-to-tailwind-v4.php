...
$patterns = [
    ...
    '/(<th[^>]*class=")([^"]*)(")/i' => function($matches) {
        ...
        if (!empty(trim($classes)) && !preg_match('/px-|py-|text-/', $classes)) {
...
function convertFile($filePath, $classMap, $patterns, $dryRun = false) {
    ...
    $dynamicPatterns = [
        '/class="([^"]*?)btn\s+{{\s*([^}]+?)\s*\?\s*\'btn-primary\'\s*:\s*\'btn-default\'\s*}}([^"]*)"/i' => function($matches) use (&$changes) {
            ...
            $baseClasses = trim($before . ' ' . $defaultClasses . ' ' . $after);
    ...
    foreach ($classMap as $bootstrap => $tailwind) {
        ...
        $replacement = function($matches) use ($bootstrap, $tailwind, &$changes) {
            $before = $matches[1];
            $after = $matches[2];
            $changes++;
            
            // Clean up extra spaces
            $newClass = trim($before . ' ' . $tailwind . ' ' . $after);
